...
export interface AcademicYearFormProps {
  mode: "create" | "edit";
  initialValues?: Partial<CreateAcademicYearDto & UpdateAcademicYearDto>;
...
export function AcademicYearForm({
  ...
  const [values, setValues] = useState<FormState>({
    name: initialValues?.name ?? empty.name,
    startDate: initialValues?.startDate ?? empty.startDate,
    endDate: initialValues?.endDate ?? empty.endDate,
    isActive: initialValues?.isActive ?? defaultActive,
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // server validation errors handled via `serverErrors` prop
    }
  };
    ...
    <Card className={cn("w-full")}>
      <CardHeader>
        <h2 className="text-lg font-semibold text-foreground">
          {mode === "create" ? "New academic year" : "Edit academic year"}
        </h2>
      </CardHeader>
      <CardContent>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormInput
              ...
              error={
                getFieldError(serverErrors, "endDate") ?? errors.endDate
              }
            />
          </div>
